<x-app-layout>
    ...
    <head>
        ...
        <title>Daily Report | Manajemen Jadwal</title>
    ...
    <main class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white shadow-lg rounded-xl p-6">
            {{-- <div class="flex justify-between items-center mb-6">
                <h2 class="text-5xl font-bold text-orange-600">📋 Daily Report</h2>
            </div> --}}

            <form method="POST" action="{{ route('save.daily') }}" class="p-6 bg-white shadow-md rounded-md space-y-4">
                @csrf
            <div class="flex gap-3 text-sm">
                    @if(empty($resultDailyReport))
                    ...
                    @if(empty($resultDailyReport))
                        <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @foreach ($dailyReport as $index => $item)
                                    <tr>
                                        {{-- <td class="px-4 py-2">
                                            {{ $item->kd_list }}
                                        ...
                                        <td class="px-4 py-2">
                                            {{ $item->aktifitas }}
                                            <input type="hidden" name="data[{{ $index }}][aktifitas]" value="{{ $item->aktifitas }}">
                                        </td>
                                        <td class="px-4 py-2">
                                            {{ $item->status }}
